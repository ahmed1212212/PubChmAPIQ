...
The **PubChemAPIQ** Python package simplifies interaction with the PubChem database via the PUG-REST API. Unlike traditional wrappers with hard-coded functions, PubChemAPIQ uses dynamic metaprogramming to generate endpoints, ensuring full coverage of the PubChem schema. It handles URL generation, automatic batching, and throttling to provide a seamless data retrieval experience.

---

## Naming Convention

Functions in **PubChemAPIQ** follow a strict semantic naming convention to eliminate ambiguity:
...
def test_PubChemAPIQ():
    compound = "Aspirin"
    print("Testing PubChemAPIQ functions for:", compound)
...
```text
Testing PubChemAPIQ functions for: Aspirin
...
def test_cid_functions():
    ...
    print(f"Testing PubChemAPIQ CID functions for CID: {cid}")
...
```text
Testing PubChemAPIQ CID functions for CID: 2244
...
```python
# test_PubChemAPIQ_biological.py
from PubChemAPIQ import (
    # Protein
...
def test_biological_functions():
    print("Testing PubChemAPIQ Biological Functions\n")
...
```text
Testing PubChemAPIQ Biological Functions
